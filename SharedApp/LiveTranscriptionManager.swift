...
final class LiveTranscriptionManager: ObservableObject {
    ...
    private func prepareSpeechPipeline(
        ...
        #if HAS_IOS27_SDK
        if #available(iOS 27.0, macOS 27.0, *), selectedSpeechPipelineMode == .nativeIOS27 {
    ...
    private static func makeCaptureSessionRecordingFormat(
        ...
        #else
        let sessionSampleRate = defaultCaptureDeviceSampleRate()
        #endif
    ...
    #if os(macOS)
    private static func defaultCaptureDeviceSampleRate() -> Double {
        ...
        return description.pointee.mSampleRate
    }
    #endif
...
private final class CaptureSessionRecordingPipeline: NSObject, AVCaptureAudioDataOutputSampleBufferDelegate, @unchecked Sendable {
    ...
    private func configureIfNeeded() throws {
        ...
        isConfigured = true
        #if os(iOS)
        let multichannelModeSummary = String(describing: captureInput.multichannelAudioMode)
        #else
        let multichannelModeSummary = "unavailable"
        #endif
        liveTranscriptionLogger.debug(
            "AVCapture configured device=\(device.localizedName, privacy: .public) multichannelMode=\(multichannelModeSummary, privacy: .public) file=\(liveAudioFormatSummary(self.recordingFormat), privacy: .public) analyzer=\(liveAudioFormatSummary(self.analyzerSourceFormat), privacy: .public)"
        )
    }
...
struct SpeechProcessingPipelineDiagnostics: Equatable {
    ...
    static func current(
        ...
        #if HAS_IOS27_SDK
        if #available(iOS 27.0, macOS 27.0, *), configuredMode == .nativeIOS27 {
            return SpeechProcessingPipelineDiagnostics(
