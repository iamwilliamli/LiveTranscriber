...
#define LT_HAS_LLAMA 1
#else
#define LT_HAS_LLAMA 0
#endif
...
+ (nullable NSString *)generateTextWithModelAtPath:(NSString *)modelPath
                                     ...
                                     contextTokens:(NSInteger)contextTokens
                                       temperature:(float)temperature
                                             ...
                                             error:(NSError **)error {
    if (error != nullptr) {
        *error = [NSError errorWithDomain:LocalLlamaBridgeErrorDomain
                                     code:LocalLlamaBridgeErrorModelLoadFailed
                                 userInfo:@{
                                     NSLocalizedDescriptionKey:
                                         @"The local summary engine is not bundled in this build.",
                                 }];
    }
    return nil;
}
