...
| 想快速回顾 | 用 Apple Intelligence 或本地 Qwen3 生成摘要和主题标签。 |
...
- 可选使用自己的 OpenAI API Key 做云端重转写。
- 录音详情支持回放、倍速、按时间戳跳转、翻译、复制、分享、编辑、锁定/解锁转录、删除和查看音频参数。
- 带位置信息的录音可以在地图中查看。

### 摘要、标签和搜索

- 摘要引擎可选：Automatic、Apple Intelligence、Local Qwen3。
...
- OpenAI 只在用户手动选择 OpenAI 重转写时使用，并使用用户自己的 API Key。
