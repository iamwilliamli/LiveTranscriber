...
extension MossTranscribeDiarizeModel {
    ...
    public static func fromModelDirectory(_ modelDir: URL) async throws -> MossTranscribeDiarizeModel {
        #if os(iOS)
        // Metal's recommended working-set size can be larger than the iOS
        // jetsam budget. Keep MLX's reusable temporary-buffer cache small so
        // loading the full checkpoint does not retain a second large working
        // set alongside the model weights.
        Memory.cacheLimit = 20 * 1024 * 1024
        Memory.clearCache()
        print("MOSS_IOS_MEMORY_CONFIG cache_limit=\(Memory.cacheLimit)")
        #endif
        ...
        // Converted MLX checkpoints store packed integer weights alongside
        // `scales` and `biases`. Replace only matching modules with their
        // quantized implementations before strict parameter verification.
        // The Whisper encoder and VQ adaptor remain BF16 in the published MOSS
        // 4-bit/8-bit checkpoints and therefore have no `scales` companions.
        ...
        model.train(false)
        #if os(iOS)
        print("MOSS_IOS_MEMORY_BEFORE_EVAL \(Memory.snapshot())")
        #endif
        eval(model)
        #if os(iOS)
        Memory.clearCache()
        print("MOSS_IOS_MEMORY_AFTER_EVAL \(Memory.snapshot())")
        #endif
        return model
    }
