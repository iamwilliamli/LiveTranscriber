...
- [Recording Processing Pipeline](RECORDING_PIPELINE.md): AVCaptureSession audio capture, transcription fan-out, stereo recording, and normalization decisions.
...
3. `RecordingStore` owns saved recording metadata, local app-private storage by default, optional app-private iCloud storage, SwiftData local/CloudKit private index persistence, import transcription, re-transcription, normalization, deletion, search inputs, and Apple Intelligence analysis.
